...
const buckmintOrganizationKey = process.env.BUCKMINT_ORGANIZATION_KEY
const buckmintApiKey = process.env.BUCKMINT_API_KEY

describe('buckmint Connector', () => {
      ...
      describe('Initiate and Process Internal Transfers', () => {
        it('Initiate Internal Transfer - 200', async () => {
          ...
          const res = await client.initiateInternalTransfer({
            organization_key: buckmintOrganizationKey as string,
            api_key: buckmintApiKey as string,
            currency: 'usdc',
            amount: 1,
      ...
      describe('Check User Existence API', () => {
        it('Check if a user exists - 200', async () => {
          ...
          const res = await client.checkInternalTransferUserExists(
            buckmintOrganizationKey as string,
            buckmintApiKey as string,
            '0x..',
          )
          expect(res).to.have.property('status')
          expect(res.status).to.eql('success')
